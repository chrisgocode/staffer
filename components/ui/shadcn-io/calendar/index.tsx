...
export const CalendarBody = ({ events, children }: CalendarBodyProps) => {
  ...
  const eventsByDay = useMemo(() => {
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      result[day] = events.filter((event) => {
        return isSameDay(new Date(event.endTime), new Date(year, month, day));
      });
    }
    return result;
  ...
  for (let day = 1; day <= daysInMonth; day++) {
    ...
    days.push(
      ...
      >
        {day}
        <div>
          {eventsForDay.slice(0, 3).map((event) => children({ event }))}
        </div>
...
export const CalendarItem = memo(({ event, className }: CalendarItemProps) => (
  <div className={cn("flex items-center gap-2", className)}>
    <div
